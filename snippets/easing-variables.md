...
可以被 `transition-timing-function` 属性重用的变量, 比用 `ease`, `ease-in`, `ease-out` 和 `ease-in-out` 创建的更强大.
...
变量使用 `:root` 这个CSS 伪类来全局定义，这样就可以匹配文档树中的根节点. 在HTML中, `:root` 代表了 `<html>` 元素和   `html`选择器是相同的,除了它的特异性要高点.
